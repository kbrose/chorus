...
def evaluate(
    ...
    preds_targets = [(model(x.to(DEVICE))[0], y, y_ind) for (x, _, _), y, y_ind in data]
    valid_loss = (
        torch.tensor([loss_fn(preds, y.to(DEVICE)) for preds, y, _ in preds_targets])
        .mean()
        .cpu()
        .numpy()
    )
...
def train(name: str):
    ...
    train_dl = torch.utils.data.DataLoader(train, BATCH, num_workers=4, pin_memory=True)
        ...
        with tqdm(desc=f"{ep: >3}", total=len(train_dl), ncols=80) as pbar:
            ...
            for (xb, _, _), yb, _ in BgGenerator(train_dl, 5):
                ...
                pbar.set_postfix_str(
                    postfix_str.format(
                        train_loss=round(np.mean(losses), 4), valid_loss="", star=" "
                    ),
                    refresh=False,
                )
            ...
            with torch.no_grad():
                ...
                if valid_loss < best_valid_loss:
                    ...
                    best_ep = ep
                    torch.jit.save(model, str(MODELS / name / f"{ep:0>4}.pth"))
                pbar.set_postfix_str(
                    postfix_str.format(
